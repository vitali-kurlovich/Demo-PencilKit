...
extension Step_O8View {
    ...
    var fillColor: Color {
        Color(red: 211.0 / 255, green: 196.0 / 255, blue: 182.0 / 255)
    }

    var fromStyle: PathStyle {
        switch state {
        case .initial:
            let fromStroke = StrokeStyle(lineWidth: 1, dash: [15, 5])
            return PathStyle(
                fill: .init(color: fillColor),
                stroke: .init(color: .indigo, style: fromStroke, widthMode: .absolute),
            )
        case .started, .highlight:
            let fromStroke = StrokeStyle(lineWidth: 2, dash: [15, 5])
            return PathStyle(
                fill: .init(color: fillColor.opacity(0.5)),
                stroke: .init(color: .indigo, style: fromStroke, widthMode: .absolute),
            )
        }
    }

    var toStyle: PathStyle {
